...
import math
import zelava_grafika

from letters import filtered_letter_objects, text_width


class TextTurtle(zelava_grafika.Turtle):
    ...
    def _draw_letter(self, letter_obj, spacing):
        ...
        for i, p in enumerate(p_data):
            ...
            self.head_to(*p)
        self.penup()
        xy_start += zelava_grafika.Vec2D(segment_length, 0).rotate(base_heading)
...
def _letter_to_baseline_coords(letter, height, base_xy, base_heading):
    ...
    for point in letter.data:
        ...
        ly = y
        # rotate it
        xy = zelava_grafika.Vec2D(lx, ly).rotate(base_heading)
        # translate to the start position
        delta_left = letter.get_delta_left(1)
        width = letter.get_width(1)
        xy += zelava_grafika.Vec2D(xmin + delta_left, 0).rotate(base_heading)
        # now with height
        hxy = zelava_grafika.Vec2D(*xy) * height
        # translate along the baseline
        tx, ty = zelava_grafika.Vec2D(*hxy) + zelava_grafika.Vec2D(*base_xy)
